...
void sensorsDeviceInit(void)
{
	...
	{
		isMPUPresent=true;
		rt_kprintf("MPU9250 I2C connection [OK].\n");
	}
	else
	{
		rt_kprintf("MPU9250 I2C connection [FAIL].\n");
	}
	...
	if (ak8963TestConnection() == true)
	{
		...
		rt_kprintf("AK8963 I2C connection [OK].\n");
	}
	else
	{
		rt_kprintf("AK8963 I2C connection [FAIL].\n");
	}
#endif
...
void sensorsTask(void *param)
{
	...
	while (1)
	{
		if (pdTRUE == xSemaphoreTake(sensorsDataReady, portMAX_DELAY))
		{
			...
			vTaskSuspendAll();	/* 禁止调度， 确保同一时刻把数据放入队列中*/ 
			xQueueOverwrite(accelerometerDataQueue, &sensors.acc); // 往队列尾部覆盖写入
...
/*  在HAL库中，需要编写HAL_GPIO_EXTI_Callback（uint16_t GPIO_Pin） 回调函数
 *	识别是哪个IO口发出的,做出相关的处理。
 */
