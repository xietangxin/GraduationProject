...
void ledseqInit()
{
	...
	for(i=0; i<LED_NUM; i++)
		...
		//timer[i] = xTimerCreate("ledseqTimer", 1000, pdFALSE, (void*)i, runLedseq);  // 创建单次1000时钟节拍，编号为i的软件定时器
...
void ledseqRun(led_e led, const ledseq_t *sequence)
{
	...
	//xSemaphoreTake(ledseqSem, portMAX_DELAY); // 阻塞等待没有超时限制
	state[led][prio] = 0; 
	updateActive(led);
	
	rt_sem_release(&ledseqSem);
	//xSemaphoreGive(ledseqSem);  // 释放信号量
...
void runLedseq(void *paramter)
{
	...
	//led_e led = (led_e)pvTimerGetTimerID(xTimer);  // 获取定时器ID
